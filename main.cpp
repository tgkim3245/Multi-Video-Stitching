...
Mat stitchFrames(Mat left, Mat right) {
  // 두 이미지가 모두 유효한지 확인
  ...
  // 이미지 크기를 동일하게 조정
  int height = min(left.rows, right.rows);
  left = left(Rect(0, 0, left.cols, height));
  right = right(Rect(0, 0, right.cols, height));

  // 특징점 감지 및 매칭
  Ptr<cv::ORB> orb = cv::ORB::create(5000);
  ...
  Mat H = findHomography(src_pts, dst_pts, RANSAC, 4.0);
  ...
  // 호모그래피 변환 후 left의 특징점이 어디로 이동했는지 계산
  vector<Point2f> transformed_src_pts;
  perspectiveTransform(src_pts, transformed_src_pts, H);

  // right가 배치될 위치 계산 (특징점 기반)
  // transformed_src_pts[i].x = right_x + dst_pts[i].x 가 되어야 함
  double right_x_sum = 0;
  int valid_matches = 0;
  for (int i = 0; i < min((int)transformed_src_pts.size(), (int)dst_pts.size());
       i++) {
    if (transformed_src_pts[i].x >= -100 &&
        transformed_src_pts[i].x < 2000) { // 유효한 범위
      right_x_sum += transformed_src_pts[i].x - dst_pts[i].x;
      valid_matches++;
    }
  }
  ...
  if (valid_matches == 0)
    valid_matches = 1;
  int right_x = (int)(right_x_sum / valid_matches);

  cout << "[STITCH] Right offset: " << right_x << endl;

  // 캔버스 크기 결정: left와 right 전체가 다 들어가야 함
  int output_width =
      left.cols + right.cols; // 최악의 경우 겹치지 않으면 이 크기
  int output_height = max(left.rows, right.rows);

  // 변환된 left 이미지 생성
  Mat warped = Mat::zeros(output_height, output_width, CV_8UC3);
  warpPerspective(left, warped, H, warped.size());

  // result 생성: warped (left 전체)를 기반으로 시작
  Mat result = warped.clone();

  // right를 calculated offset 위치에 배치 (전체 right 표시)
  if (right_x >= -right.cols && right_x < output_width) {
    // right_x가 유효한 범위 내
    int copy_src_x = 0;
    int copy_dst_x = right_x;
    int copy_width = right.cols;

    // 왼쪽이 캔버스 바깥이면 자르기
    if (right_x < 0) {
      copy_src_x = -right_x;
      copy_dst_x = 0;
      copy_width = right.cols - copy_src_x;
    }

    // 오른쪽이 캔버스 바깥이면 자르기
    if (copy_dst_x + copy_width > output_width) {
      copy_width = output_width - copy_dst_x;
    }

    if (copy_width > 0 && copy_src_x < right.cols) {
      Mat right_roi = right(Rect(copy_src_x, 0, copy_width, right.rows));
      right_roi.copyTo(result(Rect(copy_dst_x, 0, copy_width, right.rows)));
    }
  }
